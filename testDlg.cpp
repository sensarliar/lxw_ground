...
//	GetDlgItem(IDC_BUTTON_SEND)->SetBitmap(m_bitmap);
	...
	{
		MessageBox("ÎÄ¼þ´ò¿ªÊ§°Ü", "GroundRecord001.txt", MB_OK);
		return false;
	}
...
bool CTestDlg::InintCom(int port)
{
	...
	if(!m_com.open(port))//´ò¿ª´®¿Ú2
	{
		MessageBox("´®¿Ú´ò¿ªÊ§°Ü", "COM15", MB_OK);
		return false;
	}
...
/*
		if(m_wei.GetAt(0)=='N')
			m_wei="N";
		else m_wei="W";//¶¨Î»ÐÅÏ¢ÌáÈ¡
		*/
		SetDlgItemText(IDC_Wei,m_wei);
					
		
		m_jingdu=strBuffer.Mid(index_start+21,3);//¾­¶ÈÐÅÏ¢ÌáÈ¡
		...
		if(m_jing.GetAt(0)=='W')
			m_jing="W";
		else m_jing="¶«¾­";//¶¨Î»ÐÅÏ¢ÌáÈ¡
		*/
		SetDlgItemText(IDC_Jing,m_jing);
//+1

		m_haiba=strBuffer.Mid(index_start+34,5);//º£°ÎÐÅÏ¢ÌáÈ¡
		SetDlgItemText(IDC_Haiba,m_haiba);


		m_day=strBuffer.Mid(index_start+39,2);
		SetDlgItemText(IDC_Day,m_day);    //ÈÕÆÚ
		...
		m_speed=strBuffer.Mid(index_start+45,index_comma_dot+2-index_start-45);   //ËÙ¶È
		SetDlgItemText(IDC_SPEED,m_speed);
		index_comma_pre=index_comma+1;

		index_comma = strBuffer.Find(',',index_comma_pre);
		m_jihao=strBuffer.Mid(index_comma_pre,index_comma-index_comma_pre);   //·É»úºÅ
